...
use crate::substrato_4004::mock::{
    EthicalFilter, Action, ComplianceError, FilterVerdict, LayerViolation, OrchestratorEvent,
    EventStore, B20Constants, extract_address, IB20, Provider, Http
};
...
impl ComplianceEngine {
    ...
    async fn check_policies(&self, op: &B20Operation) -> Result<PolicyCompliance, ComplianceError> {
        match op {
            B20Operation::Transfer { token, from, to, .. } => {
                let sender_policy = self.policy_registry.get_policy(*token, PolicyScope::TransferSender).await.map_err(|_| ComplianceError::Other("".into()))?;
                let receiver_policy = self.policy_registry.get_policy(*token, PolicyScope::TransferReceiver).await.map_err(|_| ComplianceError::Other("".into()))?;

                let sender_ok = self.policy_registry.is_authorized(sender_policy, *from).await.map_err(|_| ComplianceError::Other("".into()))?;
                let receiver_ok = self.policy_registry.is_authorized(receiver_policy, *to).await.map_err(|_| ComplianceError::Other("".into()))?;
            ...
            B20Operation::Mint { token, to, .. } => {
                let policy = self.policy_registry.get_policy(*token, PolicyScope::MintReceiver).await.map_err(|_| ComplianceError::Other("".into()))?;
                if !self.policy_registry.is_authorized(policy, *to).await.map_err(|_| ComplianceError::Other("".into()))? {
    ...
    async fn check_roles(&self, op: &B20Operation, _action: &Action) -> Result<RoleCompliance, ComplianceError> {
