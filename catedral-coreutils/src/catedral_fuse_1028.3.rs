...
/// Wrapper em torno de FileType para poder serializar
...
impl From<FileType> for CathedralFileType {
...
impl Into<FileType> for CathedralFileType {
...
impl Filesystem for CathedralFS {
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        let mut theosis_ok = true;
        let mut theosis_val = 0.0;

        if let Some(node) = self.nodes.values().find(|n| n.parent == parent && n.name == name_str) {
            if !node.check_theosis(self.theosis_threshold) {
                theosis_ok = false;
            } else {
                found_attr = Some(node.to_attr());
                theosis_val = node.theosis;
            }
        }
...
fn main() {
    let mountpoint = std::env::args().nth(1).unwrap_or_else(|| "/mnt/catedral".to_string());
