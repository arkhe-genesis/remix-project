...
pub async fn handle_create_context(
    ...
    let ctx: Arc<Context> = manager.create_context(&id).await;
    // Configura limites
...
pub async fn handle_add_to_context(
    ...
    let ctx: Arc<Context> = manager.get_context(context_id).await
        .ok_or_else(|| MCPError {
            code: -32000,
    ...
    let _ = ctx.add(content, role, importance).await
        .map_err(|e| MCPError {
            code: -32000,
...
pub async fn handle_get_context(
    ...
    let entries: Vec<crate::orchestrator::context::ContextEntry> = ctx.get_entries().await;
    let metadata = ctx.metadata().await;
    ...
    Ok(json!({
        ...
        "metadata": {
            ...
            "total_tokens": metadata.total_tokens,
            "compression_ratio": metadata.compression_ratio,
            "oldest_entry": metadata.oldest_entry.map(|d: String| d),
            "newest_entry": metadata.newest_entry.map(|d: String| d),
        },
