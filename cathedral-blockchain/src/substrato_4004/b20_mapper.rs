...
use crate::substrato_4004::mock::{
    Address, U256, Bytes, Action, EthicalFilter, FilterVerdict, MapperError, LayerViolation,
    extract_address, extract_u256, extract_optional_memo, extract_policy_scope, extract_u64,
    extract_pausable_features, hash_memo,
};
...
impl B20TokenMapper {
        ...
        match action.action_type.as_str() {
            "payment_b20" => {
                ...
                let sender_policy = self.policy_registry
                    ...
                    .await.map_err(|_| MapperError::ExtractionError)?;

                if !self.policy_registry.is_authorized(sender_policy, from).await.map_err(|_| MapperError::ExtractionError)? {
            ...
            "freeze_and_seize" => {
                ...
                if self.policy_registry.is_authorized(sender_policy, target).await.map_err(|_| MapperError::ExtractionError)? {
    ...
    async fn get_total_supply(&self, _token: Address) -> Result<U256, MapperError> { Ok(U256([0; 4])) }
    async fn get_supply_cap(&self, _token: Address) -> Result<U256, MapperError> { Ok(U256([u64::MAX; 4])) }
}
