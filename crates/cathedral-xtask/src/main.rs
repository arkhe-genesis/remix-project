...
fn pre_commit() -> Result<()> {
    ...
    run(
        "cargo clippy --workspace --all-features || true",
        "Lints (clippy)",
    )?;
    run("cargo deny check || true", "Dependências (deny)")?;
    run("cargo audit --deny-warnings || true", "Vulnerabilidades (audit)")?;
...
fn ci() -> Result<()> {
    ...
    run("cargo insta test --workspace || true", "Snapshot tests (insta)")?;
    run("cargo bench --workspace", "Benchmarks")?;
    run(
        "cargo llvm-cov --workspace --html --output-dir target/coverage || true",
        "Cobertura (llvm-cov)",
    )?;
    run(
    ...
    run("cargo deadlinks --check-http || true", "Links quebrados")?;
