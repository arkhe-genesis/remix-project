...
pub enum CognitiveCapability {
    Reactive, Symbolic, Planning, Episodic, Causal, Creative,
}
...
pub enum RouterStrategy {
    HighestConfidence, WeightedVoting, Concatenate, Adaptive,
}

impl Default for RouterStrategy {
    fn default() -> Self { RouterStrategy::Adaptive }
}
...
impl CognitiveRouter {
    pub fn new() -> Self { Self { strategy: RouterStrategy::default() } }
    pub async fn combine(
        &self,
        outputs: Vec<CognitiveOutput>,
        ctx: &CognitiveContext,
    ) -> Result<Vec<CognitiveOutput>, String> {
        if outputs.is_empty() { return Err("Nenhum output para combinar".to_string()); }
        ...
        match strategy {
            RouterStrategy::HighestConfidence | RouterStrategy::Adaptive => {
                let best = outputs.into_iter().max_by(|a, b| a.confidence.partial_cmp(&b.confidence).unwrap()).unwrap();
                Ok(vec![best])
            }
            RouterStrategy::WeightedVoting => {
                ...
                Ok(vec![CognitiveOutput {
                    content: combined_content, tool_calls: combined_tools, confidence: total_conf / len,
                    thinking_trace: None, source_expert: "combined".to_string(),
                }])
            }
            RouterStrategy::Concatenate => {
                let mut combined = CognitiveOutput {
                    content: String::new(), tool_calls: Vec::new(), confidence: 0.0,
                    thinking_trace: None, source_expert: "combined".to_string(),
                };
                let mut total_conf = 0.0;
                let len = outputs.len() as f64;
                ...
                combined.confidence = total_conf / len;
                Ok(vec![combined])
            }
        }
    }
...
impl MoeCognitiveOrchestrator {
    pub fn new() -> Self {
        Self { experts: Vec::new(), router: CognitiveRouter::new(), expert_capacity: HashMap::new() }
    }
    ...
    pub async fn route_and_process(&self, ctx: &CognitiveContext) -> Result<Vec<CognitiveOutput>, String> {
        let mut scores: Vec<_> = self.experts.iter().map(|e| (e.id(), e.activation_score(ctx))).collect();
        scores.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
        let k = match ctx.consciousness {
            ConsciousnessState::Dormant => 1, ConsciousnessState::Aware => 2, ConsciousnessState::Reflective => 3,
            ConsciousnessState::MetaCognitiva => 4, ConsciousnessState::Autopoiética => 5,
        };
        ...
        while let Some(result) = handles.join_next().await {
            if let Ok((_, Ok(output))) = result { outputs.push(output); }
        }
        if outputs.is_empty() { return Err("Nenhum expert processou a requisição".to_string()); }
