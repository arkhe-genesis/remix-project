...
pub enum MessageType {
    Task,           // Solicitação de execution
    Result,         // Resultado de task
    Query,          // Consulta de status
    Notification,   // Notificação sem reposta
    Handshake,      // Estabelecimento da comunicacao
    Heartbeat,      // Prova de vida
    Terminate,      // Solicitação de finalizacao
}
...
impl MessageEnvelope {
    ...
    pub fn mark_delivered(&mut self) {
        self.delivered_at = Some(Utc::now().to_rfc3339());
    }
